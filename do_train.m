...
load(TRAINING_DATA); % this will load 'data'
...
stacked_data = zeros(numImgs * SEG.nC, size(data{1}.feat, 2));

for img_ind = 1:numImgs
    base_index = (img_ind - 1) * SEG.nC;
    
    ground_truth(((base_index+1):(base_index+SEG.nC)), :) = data{img_ind}.label;
